"""Remove notification table

Revision ID: 5a47cc5c1775
Revises: f5f8924f6b08
Create Date: 2018-10-01 20:55:44.402166

"""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import mysql

# revision identifiers, used by Alembic.
revision = '5a47cc5c1775'
down_revision = 'f5f8924f6b08'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index('ix_notification_created', table_name='notification')
    op.drop_index('ix_notification_name', table_name='notification')
    op.drop_table('notification')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('notification',
    sa.Column('id', mysql.INTEGER(display_width=11), autoincrement=True, nullable=False),
    sa.Column('name', mysql.VARCHAR(length=128), nullable=True),
    sa.Column('user_id', mysql.INTEGER(display_width=11), autoincrement=False, nullable=True),
    sa.Column('created', mysql.FLOAT(), nullable=True),
    sa.Column('payload_json', mysql.TEXT(), nullable=True),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], name='notification_ibfk_1'),
    sa.PrimaryKeyConstraint('id'),
    mysql_default_charset='utf8',
    mysql_engine='InnoDB'
    )
    op.create_index('ix_notification_name', 'notification', ['name'], unique=False)
    op.create_index('ix_notification_created', 'notification', ['created'], unique=False)
    # ### end Alembic commands ###
